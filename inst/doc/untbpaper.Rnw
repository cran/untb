...
<<SampleTenThousand,echo=FALSE,print=FALSE,cache=TRUE>>=
set.seed(0)
...
\begin{figure}[htb]
\begin{center}
<<PlotSampleTenThousand,fig=TRUE>>=
pa <- preston(a,n=12)
...
@ 
\caption{Preston plot \label{prestonplotsynthetic} of a synthetic
  dataset of~$J=10000$ individuals, partially isolated from the
  metacommunity ($\jm=5\times 10^6$, $\theta=50$; $m=0.01$) and
  allowed to simulate for~$10^4$ deaths per individual, with grey bars
  showing the binned abundance classes.  Superimposed lineplots show
  expected number of species for each abundance class
  following~\citet{volkov2003}, using estimated (maximum likelihood;
  $\theta=41.83$, $m=0.06$; red) and true parameters (blue)
  respectively}
\end{center}
\end{figure}

Figure~\ref{prestonplotsynthetic} shows another type of analysis
performed on a synthetic dataset.  The expected number of species,
calculated using the methods of~\cite{volkov2003}, are shown
superimposed on a Preston plot.  Two expectations are given: one
calculated using the true parameters (blue crosses), and one using
estimated values (red circles).  The difference between these two
expectations shows the sensitivity to the assumed values of the
parameters.  Note that the ordinates of the red curve are (correlated)
random variables; further work might include using the package to
assess their covariance.
